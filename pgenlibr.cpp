...
void PgenReader::Load(std::string filename, uint32_t cur_sample_ct, std::vector<int> sample_subset_1based, int nthr) {
  ...
  for(int i = 0; i < nthr; i++) {
    ...
    if (reterr != plink2::kPglRetSuccess) {
      ...
      snprintf(errstr_buf, 256, "PgrInit() error %d", static_cast<int>(reterr));
      fprintf(stderr,"%s\n", errstr_buf);
      exit(-1);
    }
...
void PgenReader::ReadIntHardcalls(std::vector<int>& buf, int const& thr, int variant_idx, int allele_idx) {
  if (!_info_ptr) {
    fprintf(stderr, "pgen is closed");
    exit(-1);
  }
  ...
  if (buf.size() != _subset_size[thr]) {
    buf.resize(_subset_size[thr]);
  }
